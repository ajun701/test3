...
def find_column_with_fallback(
    df: pd.DataFrame, candidates: List[str], fuzzy_keywords: Optional[List[str]] = None
) -> Optional[str]:
    ...
    for c in df.columns:
        ...
        if any(str(k).lower() in name for k in fuzzy_keywords):
            return c
    return None
...
def validate_row_detail(
    ...
    account_ok = bool(acct and (REGEX_PHONE.match(acct) or REGEX_EMAIL.match(acct)))
    if not account_ok:
...
def validate_row(
    ...
    ok, reason, _ = validate_row_detail(amount, alipay_account, alipay_name, logistics_no)
    return ok, reason


def build_row_identity_keys(
    df: pd.DataFrame, id_col: str, order_col: str, logistics_col: str
...
def compare_source_and_processed(
    source_df: pd.DataFrame, processed_df: pd.DataFrame, stage_name: str
) -> Dict[str, Any]:
    """生成行数一致性校验报告。"""
