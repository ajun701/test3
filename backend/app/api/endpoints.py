...
from app.models import AITask, OperationHistory, User
...
def save_artifact(file_bytes: bytes, prefix: str, suffix: str = ".xlsx") -> str:
    """保存生成的 Excel 文件，并返回供前端下载的相对 URL 路径"""
...
async def clean_data(
    ...
    try:
        ...
        url_abnormal = save_artifact(b_abnormal, "clean_abnormal_need_callback")
...
async def match_data(
    ...
    try:
        ...
        url_inbound = save_artifact(b_inbound, "matched_inbound_for_ai")
        url_pending = save_artifact(b_pending, "not_inbound_followup")
...
def get_ai_task_status(task_id: str, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def export_ai_task_snapshot(task_id: str, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    task = db.query(AITask).filter(AITask.task_id == task_id).first()
    if not task:
        raise HTTPException(status_code=404, detail="任务不存在")
    ...
    if not df_bad.empty:
        ...
        url_bad = save_artifact(b_bad, "ai_bad_snapshot")
...
def pause_ai_task(task_id: str, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    task = db.query(AITask).filter(AITask.task_id == task_id).first()
    if not task:
        raise HTTPException(status_code=404, detail="任务不存在")
...
@router.get("/history/{history_id}/artifacts", summary="获取历史记录关联的产物文件列表")
def get_history_artifacts(
    history_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    record = db.query(OperationHistory).filter(OperationHistory.id == history_id).first()
    if not record:
